...
if [ $(id -u) == 0 ] ; then
    ...
    if [[ $NB_USER != "atlas-coffea" ]]; then
	groupadd "atlas-af" -g 5741 
	useradd -c "$NB_UNAME" -u "$NB_UID" -M -b "/home" -N -g atlas-af -G atlas-af "$NB_USER"
	cd /home/$NB_USER
    fi
    #/usr/local/bin/sync_users_debian.sh -u root.atlas-af -g root.atlas-af -e https://api.ci-connect.net:18080 --no-create-dirs
    #unset API_TOKEN
